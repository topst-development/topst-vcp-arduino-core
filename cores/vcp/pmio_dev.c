...
/*PMIO EXTRA FEATURE=====================================================*/
    ...
    /* You can turn off the backup ram clear function in backup ram test mode.*/
    /* The default always clears the backup ram.*/
    //#define PMIO_SUBFEATURE_DEV_TEST_CLEAR_BACKUP_RAM
...
#define PMIO_VA_ACC_OFF_TIME_LIMIT      (10UL)
    ...
    #define PMIO_VA_RTC_LIMIT_SEC       (30UL)
...
)
{
    ...
    #else
    uiSwDt  = SAL_ReadReg(PMIO_VA_PMU_ECID_USER0_FBOUT0_ADDR);
    uiAwoDt = SAL_ReadReg(PMIO_VA_PMU_ECID_USER0_FBOUT1_ADDR);
    #endif
...
)
{
    ...
    switch(tMode)
    {
        ...
        {
            PMIO_D("[PMIO: INFO ] mode PMIO_MODE_POWER_DEEP_DOWN\n");
            break;
        }
        default :
        {
                ...
                if(gtPMIONoti.uiTaskId == 0)
                {
                                    ...
                                    NULL)) == SAL_RET_SUCCESS)
                    {
                        ...
                        if ((SAL_EventCreate((uint32_t *)&(gtPMIONoti.uiEventId), \
                                        (const uint8_t *)"msg signal event created", 0)) != SAL_RET_SUCCESS)
                        {
